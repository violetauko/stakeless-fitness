      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12 text-center">
          <div className="flex items-center justify-center mb-4">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mx-4">WHAT OUR MEMBERS SAY</h2>
            <div className="flex-1 h-0.5 bg-red-500 max-w-32"></div>
          </div>
        ...
        <div className="relative flex items-center justify-center min-h-[600px]">
                ...
                >
                  <div className="flex items-center space-x-3 w-64 bg-white/90 backdrop-blur-sm rounded-full px-4 py-3 shadow-lg hover:shadow-2xl transition-all duration-500 group-hover:bg-white border border-white/30">
                    <div className="text-right flex-1 pr-2">
                      <h3 className="font-semibold text-gray-800 text-sm mb-1 group-hover:text-red-700 transition-colors leading-tight">
                        {review.name}
                      </h3>
                      <p className="text-gray-600 text-xs mb-2 group-hover:text-gray-700 transition-colors leading-snug">
                        {review.text}
                      </p>
          ...
          <div className="relative z-10">
            <div className="w-80 h-80 md:w-96 md:h-96 relative group">
              <img
          ...
          {reviews.map((review, index) => (
            <div key={index} className="bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer group hover:scale-105">
              <div className="flex items-center space-x-4 mb-4">
                <div className="w-12 h-12 bg-gradient-to-br from-red-500 to-pink-500 rounded-full flex items-center justify-center text-white group-hover:scale-110 transition-transform duration-300">
                  {review.icon}
                </div>
                <div>
                  <h3 className="font-semibold text-gray-800 group-hover:text-red-700 transition-colors">{review.name}</h3>
                  <div className="flex space-x-1">
                    {[...Array(review.rating)].map((_, i) => (
              ...
              <p className="text-gray-600 group-hover:text-gray-700 transition-colors">{review.text}</p>
            </div>
          ))}
        </div>
          ...
          <p className="text-gray-600 mb-8 max-w-xl mx-auto">
            Join the many of satisfied members who have transformed their lives with our personalized fitness programs.
              ...
              <button><a
                ...
                className="bg-gradient-to-r from-red-600 to-pink-600 text-white px-8 py-4 rounded-full font-semibold hover:from-red-700 hover:to-pink-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
              >
                Book a Session
              </a></button>
        </div>
